...
$spy.affix(data)})})}(jQuery);;!function($){var Selectpicker=function(element,options,e){if(e){e.stopPropagation();e.preventDefault();}
...
myClasses=el;});$element.addClass('has-switch');console.log($element.data());if($element.data('on')!==undefined)
...
$.error('Method '+method+' does not exist!');};}(jQuery);$(function(){$('.switch')['bootstrapSwitch']();});;!function($){var Checkbox=function(element,options){this.init(element,options);}
...
else{window.scrollBy(0,fullTop-emptyTop);}};var init=function(){setupEvents(panels.input,applyTimeout);makePreviewHtml();if(panels.preview){panels.preview.scrollTop=0;}};init();};ui.prompt=function(title,text,defaultInputText,callback){var dialog;var input;if(defaultInputText===undefined){defaultInputText="";}
...
$(dialog).on('shown',function(){input.focus();})
$(dialog).on('hidden',function(){dialog.parentNode.removeChild(dialog);})
$(dialog).modal()},0);};function UIManager(postfix,panels,undoManager,previewManager,commandManager,helpOptions){var inputBox=panels.input,buttons={};makeSpritedButtonRow();var keyEvent="keydown";if(uaSniffed.isOpera){keyEvent="keypress";}
...
if(button.execute){button.execute(undoManager);}};function setupButton(button,isEnabled){if(isEnabled){button.disabled=false;if(!button.isHelp){button.onclick=function(){if(this.onmouseout){this.onmouseout();}
...
if(!needsRemoval)
return html;var ctag=0;html=html.replace(re,function(match){var res=tagremove[ctag]?"":match;ctag++;return res;});return html;}})();(function($){$.fn.pagedownBootstrap=function(options){var settings=$.extend({'sanitize':true,'help':null,'hooks':Array()},options);return this.each(function(){var converter=null;if(settings.sanitize)
{converter=Markdown.getSanitizingConverter();}else{converter=new Markdown.Converter()}
for(var i in settings.hooks)
{var hook=settings.hooks[i];if(typeof hook!=='object'||typeof hook.event==='undefined'||typeof hook.callback!=='function')
{continue;}
converter.hooks.chain(hook.event,hook.callback);}
var id="wmd-input";var idAppend=0;while($("#"+id+"-"+idAppend.toString()).length>0)
{idAppend++;}
$(this).attr('id',id+"-"+idAppend.toString());$(this).wrap('<div class="wmd-panel" />');$(this).before('<div class="row"><div id="wmd-button-bar-'+idAppend+'" class="wmd-button-bar" /></div>');var wrapper=$('<div class="row"/>');wrapper.append('<div class="col-xs-6"/>');wrapper.insertAfter($(this));$(this).appendTo(wrapper.find('.col-xs-6'));wrapper.append('<div class="col-xs-6"><div id="wmd-preview-'+idAppend+'" class="wmd-preview" /></div>');$(this).addClass('wmd-input');help=null;if($.isFunction(settings.help))
{help={handler:settings.help};}
var editor=new Markdown.Editor(converter,"-"+idAppend.toString(),help);editor.run();});};})(jQuery);;(function($){$.fn.tabby=function(options){var opts=$.extend({},$.fn.tabby.defaults,options);var pressed=$.fn.tabby.pressed;return this.each(function(){$this=$(this);var options=$.meta?$.extend({},opts,$this.data()):opts;$this.bind('keydown',function(e){var kc=$.fn.tabby.catch_kc(e);if(16==kc)pressed.shft=true;if(17==kc){pressed.ctrl=true;setTimeout(function(){$.fn.tabby.pressed.ctrl=false;},1000);}
...
return;}
_globalCallbacks[keys]=true;};return Mousetrap;})(Mousetrap);